...
class SimSR_sa_Agent:
    ...
    def update_simsr_sa(self, obs, action, action_seq, next_obs, reward, discount, step):
        ...
        action_en = self.SimSR_sa.act_tok(action, seq=False)
        curr_za = self.SimSR_sa.project_sa(z, action_en)

        obs_en2 = obs_en[perm]
        action2 = action[perm]
        with torch.no_grad():
            z2 = self.SimSR_sa.encode(obs_en2, ema=True)
            action_en2 = self.SimSR_sa.act_tok(action2, seq=False)
            curr_za2 = self.SimSR_sa.project_sa(z2, action_en2)
        ...
        z_cosine_dis = cosine_dis(curr_za, curr_za2, step)
        z_diff = diff(curr_za, curr_za2, z_cosine_dis, step)
        ...
        if self.use_tb:
            ...
            metrics['c1'] = c[0].item()
            metrics['c2'] = c[1].item()
            metrics['raw_c1'] = self.raw_c[0].item()
            metrics['raw_c2'] = self.raw_c[1].item()
            # metrics['KL_a'] = KL_a.mean().item()
